...
static_assert(LOG_NUM_SEVERITIES == sizeof(log_severity_names)/ sizeof(log_severity_names[0]),
...
LogMessage::~LogMessage() {
  size_t stack_start = stream_.tellp();
    ...
    !defined(OS_AIX) && !defined(OS_POSIX)
...
//      std::string main_bundle_id =std::string("");
...
void LogMessage::Init(const char* file, int line) {
  ...
  if (last_slash_pos != std::string::npos)
    filename.erase(last_slash_pos + 1);
...
std::ostream& std::operator<<(std::ostream& out, const wchar_t* wstr) {
  return out << "wide string- fix this!";
}

...
#include "base/media_log.h"
#include "mp4/fourccs.h"
#include "mp4/parse_result.h"
...
class MEDIA_EXPORT BufferReader {
 public:
  ...
  bool ReadFourCC(FourCC* v);

  bool ReadVec(std::vector<uint8_t>* t, uint64_t count);

  // These variants read a 4-byte integer of the corresponding signedness and
  // store it in the 8-byte return type.
  bool Read4Into8(uint64_t* v);
  bool Read4sInto8s(int64_t* v);

  // Advance the stream by this many bytes.
  bool SkipBytes(uint64_t nbytes);
